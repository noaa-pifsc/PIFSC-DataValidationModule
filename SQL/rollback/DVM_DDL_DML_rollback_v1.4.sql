...
create or replace PACKAGE DVM_PKG IS
	...
	*/
		...
		*/
		...
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE POPULATE_ISSUE_REC_SP (CUR_ID IN NUMBER, QC_CRITERIA_POS IN NUMBER, ISSUE_REC OUT DVM_ISSUES%ROWTYPE);
		...
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE REPLACE_PLACEHOLDER_VALS_SP (CUR_ID IN NUMBER, QC_CRITERIA_POS IN NUMBER, P_TEMPLATE_VALUE IN VARCHAR2, P_REPLACED_VALUE OUT VARCHAR2, P_APP_LINK_PARAM IN BOOLEAN);
...
--					DBMS_OUTPUT.PUT_LINE('DVM_PKG error: '||V_EXCEPTION_MSG);
		...
		EXCEPTION
			WHEN OTHERS THEN
				...
				--log the processing error:
				DB_LOG_PKG.ADD_LOG_ENTRY ('ERROR', V_TEMP_LOG_SOURCE, 'The DVM could not be evaluated successfully on the specified Parent Record:'||chr(10)||SQLERRM, V_SP_RET_CODE);
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'running PROCESS_QC_CRITERIA_SP (), OBJECT_NAME: '||PV_ALL_CRITERIA(P_BEGIN_POS).OBJECT_NAME, V_SP_RET_CODE);
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'closing cursor SRC_CUR in PROCESS_QC_CRITERIA_SP()', V_SP_RET_CODE);
		...
		--This procedure populates an issue record variable (ISSUE_REC) with the information based on the issue description and application link templates (specified by QC_CRITERIA_POS) and from the corresponding QC query result set row (specified by CUR_ID).
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE POPULATE_ISSUE_REC_SP (CUR_ID IN NUMBER, QC_CRITERIA_POS IN NUMBER, ISSUE_REC OUT DVM_ISSUES%ROWTYPE) IS
...
--							DB_LOG_PKG.ADD_LOG_ENTRY('DEBUG', V_TEMP_LOG_SOURCE, 'The current placeholder name is: '||V_ARRAY_FIELDS(j), V_SP_RET_CODE);
	...
	--							DB_LOG_PKG.ADD_LOG_ENTRY('DEBUG', V_TEMP_LOG_SOURCE, 'The current placeholder matches the current field name', V_SP_RET_CODE);
		...
		--This procedure replaces all template placeholders in P_TEMPLATE_VALUE with the corresponding values in the result set row specified by CUR_ID for the QC criteria identified by QC_CRITERIA_POS and returns the replaced value as P_REPLACED_VALUE.  If the value of the parameter P_APP_LINK_PARAM is true then this is an application link (APP_ID and APP_SESSION placeholders are ignored) and if P_APP_LINK_PARAM is false it is not an application link.  This procedure uses the DBMS_SQL package to query the database since the query fields are only known at runtime.  This procedure will validate if there are template placeholders that are not fields in the corresponding QC view and if a QC view result field has an unexpected data type.
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE REPLACE_PLACEHOLDER_VALS_SP (CUR_ID IN NUMBER, QC_CRITERIA_POS IN NUMBER, P_TEMPLATE_VALUE IN VARCHAR2, P_REPLACED_VALUE OUT VARCHAR2, P_APP_LINK_PARAM IN BOOLEAN) IS
...
DELETE FROM DB_UPGRADE_LOGS WHERE UPGRADE_APP_NAME = 'Data Validation Module' AND UPGRADE_VERSION = '1.4';

--this script will take the current version of the SPTT data validation module and upgrade it to version 0.1 of the standalone data validation module while retaining all data:


ALTER TABLE DVM_PTA_ERR_TYP_ASSOC
DROP COLUMN ERR_ASSOC_NOTES;
...
ALTER TABLE DVM_ERRORS_HIST
MODIFY (H_DATE_OF_CHANGE NULL);

ALTER TABLE DVM_ERRORS_HIST
MODIFY (H_USER_MAKING_CHANGE NULL);

ALTER TABLE DVM_ERRORS_HIST
MODIFY (H_OS_USER NULL);

ALTER TABLE DVM_ERRORS_HIST
MODIFY (ERROR_ID NULL);


DROP TRIGGER TRG_DVM_PTA_ERRORS_HIST;

DROP TABLE DVM_PTA_ERRORS_HIST CASCADE CONSTRAINTS PURGE;

DROP SEQUENCE DVM_PTA_ERRORS_HIST_SEQ;


  CREATE OR REPLACE TRIGGER "TRG_DVM_ERRORS_HIST"
  AFTER INSERT OR DELETE OR UPDATE ON "DVM_ERRORS"
...
CREATE OR REPLACE SYNONYM QC_OBJ FOR DVM_QC_OBJECTS;
CREATE OR REPLACE SYNONYM ERR_RES FOR DVM_ERR_RES_TYPES;
CREATE OR REPLACE SYNONYM PTA_ER_TYP FOR DVM_PTA_ERROR_TYPES;
CREATE OR REPLACE SYNONYM PTA_ERR FOR DVM_PTA_ERRORS;
CREATE OR REPLACE SYNONYM ERR_TYP FOR DVM_ERROR_TYPES;
CREATE OR REPLACE SYNONYM ERRS FOR DVM_ERRORS;
CREATE OR REPLACE SYNONYM DAT_STRM FOR DVM_DATA_STREAMS;
CREATE OR REPLACE SYNONYM PTA_ER_ASC FOR DVM_PTA_ERR_TYP_ASSOC;
CREATE OR REPLACE SYNONYM ERR_SEV FOR DVM_ERR_SEVERITY;

...
- DVM Version Control Information:
  ...
  - Database: 0.12 (Git tag: DVM_db_v0.12)
...
- DVM Configuration QC Queries
  - These QC queries (objects that begin with "DVM_STD_QC") are used to identify invalid DVM configuration errors that will prevent the DVM from being executed successfully (e.g. data QC view is invalid).  These configuration QC queries are intended to be executed after the DVM configuration has been changed or if there are problems encountered during DVM execution.  The DVM_STD_QC_ALL_RPT_V combines the results of all of the standard DVM configuration QC queries for convenience.  
